...
export const defaultManualValues = {
  N: "",
  P: "",
  K: "",
  temperature: "",
...
export const validateManualSoilInput = (values) => {
  const errors = {};
...
export const normalizeManualValues = (values) =>
  Object.fromEntries(Object.entries(values).map(([key, value]) => [key, Number(value)]));

export const validateSoilFile = (file, mode) => {
  if (!file) {
  ...
  if (mode === "image") {
    ...
    if (!validTypes.includes(file.type)) {
      return { valid: false, error: "Only PNG, JPG, and JPEG images are allowed." };
    }
  ...
  if (mode === "pdf") {
    ...
    if (file.size > 10 * 1024 * 1024) {
      return { valid: false, error: "PDF size must be 10MB or less." };
    }
  }
